...
model = NeuralMF(num_users, num_items).to(device)
...
model_path = os.path.join(root_path, "models", "ncf", f"mlp_drp_out.pth")
try:
...
# if I predicted the mean value, rmse of validation set would be:

mean_rating_valid = valid_df['rating'].mean()
baseline_rmse = np.sqrt(np.mean((valid_df['rating'] - mean_rating_valid) ** 2))

mean_rating_train = train_df_all['rating'].mean()
train_baseline_rmse = np.sqrt(np.mean((train_df_all['rating'] - mean_rating_train) ** 2))
